    ...
    <div className="bg-gray-800 rounded-lg shadow-lg overflow-hidden flex flex-col transition-transform duration-300 hover:transform hover:-translate-y-1">
      ...
      <div className="p-5 bg-gray-800/50 border-t border-gray-700/50 flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => onEdit(snippet)} className="p-2 text-gray-400 hover:text-white transition-colors" title="Edit snippet"><EditIcon className="w-5 h-5"/></button>
          <button onClick={() => onDelete(snippet.id)} className="p-2 text-gray-400 hover:text-red-500 transition-colors" title="Delete snippet"><TrashIcon className="w-5 h-5"/></button>
        </div>
      </div>
    </div>
  );
